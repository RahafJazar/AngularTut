...
export class UsersService {
  ...
  constructor(private iihStorageService: IihStorageService) { }


  fillData = () => {

    // const filledStatus = !!localStorage.getItem(USERS_Filled);
    const filledStatus = !!this.iihStorageService.getItem(USERS_Filled);
    if (!filledStatus) {
      const users: User[] = [
        {
          id: 'user-1',
      ...
      this.iihStorageService.addItem(USERS_LOCAL_STORAGE_KEY, JSON.stringify(users))
      this.iihStorageService.addItem(USERS_Filled, 'true');
    }
  }
  ...
  getUsers = (page: number, pageSize: number): Promise<PaginatedResponse<User>> => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        const users: User[] = JSON.parse(this.iihStorageService.getItem(USERS_LOCAL_STORAGE_KEY) || '')
  ...
  updateUser = (userUpdate: User): Promise<User> => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        const users: User[] = JSON.parse(this.iihStorageService.getItem(USERS_LOCAL_STORAGE_KEY) || '')
